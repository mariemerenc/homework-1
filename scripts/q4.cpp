...
cv::Mat median_filter(cv::Mat img, int tamanho_janela){
    int altura{img.rows}, largura{img.cols};

    cv::Mat img_filtrada = cv::Mat(img).clone();

    int pad = tamanho_janela/2;
    ...
    for(int y{0}; y < altura; ++y){
        for(int x{0}; x < largura; ++x){
            cv::Rect roi(x, y,tamanho_janela,tamanho_janela);
            cv::Mat vizinhanca = img_pad(roi);
            std::vector<uchar> pixels;
            pixels.reserve(tamanho_janela * tamanho_janela);

            for(int i{0}; i < tamanho_janela; ++i){
                for(int j{0}; j < tamanho_janela; ++j){
...
int main(int argc, char* argv[]) {
    ...
    cv::Mat img_filtrada = median_filter(img, std::stoi(argv[2]));
